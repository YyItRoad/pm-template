...
读 `<cwd>/docs/process/STATE.md`,把 5 phase 当前状态 + 上次签字 + template_sha 渲染出来,提示下一步。
...
用 `python3 -c "import sys; sys.path.insert(0, 'scripts'); from update_state import find_state_file; print(find_state_file())"` 拿到绝对路径。

如果 `find_state_file` 抛 `StateFileCorruptError`(文件不存在):
- 输出:`✗ 未找到 docs/process/STATE.md`
...
```bash
python3 scripts/update_state.py --check
```
