...
CLI 用法:
    python .claude/scripts/audit.py                       # 跑全 5 项
    python .claude/scripts/audit.py --ac-only             # 只 AC 覆盖
    python .claude/scripts/audit.py --scope-creep         # 只范围蔓延
    python .claude/scripts/audit.py --report-path X      # 写自定义报告路径
    python .claude/scripts/audit.py --check               # 只跑不写报告(返回 exit code)
"""
...
def run_audit(checks: list[str] | None = None) -> dict[str, Any]:
    ...
    # 决定 overall 等级
    level_priority = {"PASS": 0, "MEDIUM": 1, "HIGH": 2, "CRITICAL": 3}
    overall = "PASS"
