---
name: new-project
description: pm-template 唯一入口 skill。初始化 STATE.md(若缺失),引导 L1 技术栈锁确认,串行调度 phase 0→4,每个 phase 末自动跑 DoD + critic + sign-off,逐 phase 锁。中途 Ctrl+C 可用 /new-project 续跑。
---
...
`MISSING` → 拉空 STATE 模板写到 `<cwd>/docs/process/STATE.md`:
- 模板路径:`docs/process/STATE.md`(pm-template 自己仓库的)
- 用 Read tool 读全文 → Write tool 写到目标
- 写完提示:"已初始化 STATE.md,接下来:"

`EXISTS` → 解析并显示当前状态(用 `python3 scripts/update_state.py --check`)。
